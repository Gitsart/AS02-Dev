...
void homing() {
  ...
  stepper1.setZero();
  delay(2000);
  stepper1.setSpeed(600);stepper1.moveTo(-400);
  limswitch =LOW;
  delay(2000);
  while(digitalRead(limitPin) != LOW && limit_count==1) { stepper1.rotate(1);Serial.print(limswitch);Serial.println("Finding END2");}
  if(digitalRead(limitPin) == LOW && limit_count ==1) {stepper1.stop();Serial.println("END POSITION2");limit_count+=1;limswitch=HIGH;hOme =2;}
